...
# let’s first pull out all the words from all the tweets using strsplit()
...
# Let’s take a second to explore them in more detail.

# So far we’ve seen all of our data in data frames, where each column 
# has the same number of rows.  It’s nice and rectangular and easy.  
...
# object data type, but let’s ignore that for now).

# Often times we’ll see lists of lists thrown together:
...
# of words like “the” and “of”.  I had you guys download a list of stopwords 
...
# ANNOYING THING:
# stopwords looks like it’s one list of words, but it’s actually a
...
#Boom, now we’re good to go:
head(rev(sort(table(goodwords))))

# HW6 Tip:
# We saw that just looking at absolute numbers in a timeseries is *not* 
# a good way to find “spikes”.

#One solution is to look not at volume, but “velocity”, i.e. the 
...
# diff() also takes an argument called “lag” which determines
...
# Things like that, you know?  Go check ‘em out.
...
# Yeah but everything’s all mushed on top of itself and hard to read.  
# Can we see densities more easily?

# Forget “col=2”, let’s get serious.  We can use the rgb(r, g, b) function
# to set our colors.  To do a slightly gentler red, we could use 
# col=rgb(200, 0, 0, maxColorVal=255).  
# ANNOYING THING:
# It’d be great if rgb(200, 0, 0) worked, but rgb() actually expects 
...
# rule out locations that aren’t really cities.  Cool.

# First step to getting city names:  Let’s split people’s locations 
# up on the first “,”, assuming that they list their cities first.
...
# But we’ve just printed the results, not stored them.  Also, for 
...
# of the answers.  Let’s go back to our students list we created
lapply(students, names)

# Then we can use unlist() or the gentler sapply(), which tries to unlist
# things in a smart way for you.

# Well we can loop over our lists, that’s great, but what if we want to do 
...
# we’ll quickly run into issues with Unicode characters, punctuation, 
...
#grep(pattern, data):  Searches for “pattern” in the “data”, returns
                       # the rows where it’s found.
#substr(text, start, finish):  Returns the substring of “text” from “start” to “finish”
#strsplit(data, splitter):  Splits the data based on “splitter”

# But here's a new one:
# gsub(pattern, replace, data):  Replaces every occurrence of “pattern” 
                                # with “replace” in the data.

gsub("Obama", "Romney", "Now, Obama’s a guy who knows what to do for this country")

# “pattern” can be a regular expression (remember those?) or a special character, 
...
# spaces with a single space.  “\\s” indicates a whitespace character, 
# and “\\s+” is a regular expression for “one or more whitespaces” 
# (brush up on your regexps if this looks funny)
gsub("\\s+", " ", "Collapse   all the   spaces!") 

#While we’re at it, let’s just go ahead and write a function to do
...
# the world.cities city names, removing any that don’t match.

# Well, as you might expect by now, there’s a function for that in R:  match()!
...
#We’re introducing a LOT of stuff here.  cbind() stands for “column bind” and is another way to add columns to a data frame.
...
# Cool!  We just counted the words up for each city.  What’d we get?
...
# did before with stopwords, but tm’s way cooler.
